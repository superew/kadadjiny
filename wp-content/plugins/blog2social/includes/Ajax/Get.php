...
class Ajax_Get {
    ...
    public function getShipItem() {
        ...
        if (isset($_POST['postId']) && (int) $_POST['postId'] > 0 && isset($_POST['networkId']) && (int) (int) $_POST['networkId'] > 0 && isset($_POST['networkAuthId']) && (int) $_POST['networkAuthId'] > 0) {
            ...
            if ((int) $_POST['networkId'] == 19 && isset($_POST['networkTosGroupId']) && !empty($_POST['networkTosGroupId'])) {
                ...
                if ($options->existsValueByKey((int) $_POST['postId'], sanitize_text_field(wp_unslash($_POST['networkTosGroupId'])))) {
            ...
            if (isset($_POST['b2sIsDraft']) && (int) $_POST['b2sIsDraft'] == 1) {
                ...
                if ($wpdb->get_var("SHOW TABLES LIKE '{$wpdb->prefix}b2s_posts_drafts'") == $wpdb->prefix . 'b2s_posts_drafts') {
                    ...
                    if ($drafts !== false && isset($drafts['b2s']) && !empty($drafts['b2s']) && array_key_exists(sanitize_text_field(wp_unslash($_POST['networkAuthId'])), $drafts['b2s'])) {
                        $b2sDraftData = $drafts['b2s'][sanitize_text_field(wp_unslash($_POST['networkAuthId']))];
    ...
    public function getPublishPostData() {
        ...
        if (isset($_POST['postId']) && (int) $_POST['postId'] > 0) {
            ...
            $type = (isset($_POST['type']) && in_array($_POST['type'], array('publish', 'notice', 'metrics'))) ? sanitize_text_field(wp_unslash($_POST['type'])) : 'publish';
    ...
    public function getNavbarItem() {
        ...
        if (isset($_POST['networkId']) && isset($_POST['networkAuthId']) && isset($_POST['networkType']) && isset($_POST['displayName']) && isset($_POST['mandandId']) && isset($_POST['displayName']) && !empty($_POST['displayName'])) {
            ...
            $data = array(
                ...
                'networkUserName' => isset($_POST['displayName']) ? sanitize_text_field(wp_unslash($_POST['displayName'])) : '',
                ...
                'networkTosGroupId' => (isset($_POST['networkTosGroupId']) && !empty($_POST['networkTosGroupId'])) ? trim((sanitize_text_field(wp_unslash($_POST['networkTosGroupId'])))) : '',
                'expiredDate' => wp_date('Y-m-d', strtotime('+3 days'), new DateTimeZone(date_default_timezone_get()))
            );
        ...
        if (isset($userSchedData['time'])) {
            if (is_array($userSchedData) && isset($userSchedData['delay_day']) && isset($userSchedData['time']) && is_array($userSchedData['time'])) {
                foreach ($userSchedData['time'] as $k => $v) {
                    ...
                    $userSchedData['time'][$k] = wp_date($slug, strtotime(wp_date('Y-m-d ' . $v . ':00', null, new DateTimeZone(date_default_timezone_get()))), new DateTimeZone(date_default_timezone_get()));
                }
            ...
            if ($wpdb->get_var("SHOW TABLES LIKE '{$wpdb->prefix}b2s_post_sched_settings'") == $wpdb->prefix . 'b2s_post_sched_settings') {
                ...
                if (!empty($saveSchedData) && is_array($saveSchedData)) {
                    foreach ($saveSchedData as $k => $v) {
                        $slug = ($lang == 'en') ? 'h:i A' : 'H:i';
                        $userTimes[$v->network_id][$v->network_type] = wp_date($slug, strtotime(wp_date('Y-m-d ' . $v->sched_time . ':00', null, new DateTimeZone(date_default_timezone_get()))), new DateTimeZone(date_default_timezone_get()));
                    }
                    if (!empty($userTimes)) {
    ...
    public function searchUser() {
        ...
        if (isset($_GET['search_user']) && !empty($_GET['search_user'])) {
            $options = B2S_Tools::searchUser(sanitize_text_field(wp_unslash($_GET['search_user'])));
    ...
    public function checkDraftExists() {
        ...
        if (isset($_GET['postId']) && (int) $_GET['postId'] > 0) {
            global $wpdb;
            if ($wpdb->get_var("SHOW TABLES LIKE '{$wpdb->prefix}b2s_posts_drafts'") == $wpdb->prefix . 'b2s_posts_drafts') {
                $draftEntry = $wpdb->get_var($wpdb->prepare("SELECT `id` FROM `{$wpdb->prefix}b2s_posts_drafts` WHERE `blog_user_id` = %d AND `post_id` = %d AND `save_origin` = %d", B2S_PLUGIN_BLOG_USER_ID, (int) $_GET['postId'], 0));
    ...
    public function loadInsights() {
        ...
        //Input is safely handlded and sanitized with b2s function
        // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        $filterDates = ((isset($_GET['filter_dates']) && is_array($_GET['filter_dates']) && !empty($_GET['filter_dates'])) ? B2S_Tools::sanitize_array(wp_unslash($_GET['filter_dates'])) : array());
